...
#define WRITE_PRIVATE() file << "private:\\\n"
	...
	void CodeGenerateHeader::WriteMemberProperties(const ReflectContainerData& data, std::ostream& file, const std::string& currentFileId, const CodeGenerateAddtionalOptions& addtionalOptions)
	{
		...
		if (data.Members.size())
		{
			...
			for (const auto& member : data.Members)
			{
				...
				file << "\tstatic void __READ__" << member.Name << "(Reflect::IUnserialiser& u, void* field) { DispatchReadProperty(u, \"" << member.Name << "\", *reinterpret_cast<" << member.Type << "*>(field)); }\\\n";
				file << "\tstatic void __WRITE__" << member.Name << "(Reflect::ISerialiser& s, const void* field) { DispatchWriteProperty(s, \"" << member.Name << "\", *(reinterpret_cast<const " << member.Type << "*>(field))); }\\\n";
			}
		}
	...
	void CodeGenerateHeader::WriteFunctions(const ReflectContainerData& data, std::ostream& file, const std::string& currentFileId, const CodeGenerateAddtionalOptions& addtionalOptions)
	{
		...
		WRITE_PUBLIC();
			...
			file << "\tvoid Serialise(Reflect::ISerialiser &s) const override { DispatchWriteElement(s, *this); }\\\n";
			file << "\tvoid Unserialise(Reflect::IUnserialiser& u) override   { DispatchReadElement(u, *this); }\\\n";
		WRITE_PRIVATE();
	...
	void CodeGenerateHeader::WriteStaticEnum(const ReflectContainerData& data, std::ostream& file, const std::string& currentFileId, const CodeGenerateAddtionalOptions& addtionalOptions)
	{
		...
		const auto valueType = *Util::TryGetPropertyValue(data.ContainerProps, "ValueType");
		...
		{
			auto displayLabel = Util::TryGetPropertyValue(c.Flags, "DisplayLabel");
			if (!displayLabel)
			{
				displayLabel = c.Name;
			}
...
\tconst auto r = const_cast<Reflect::Enum&>(StaticEnum).TryParse<decltype(" << data.Name << "::Value)>(value);\\\n\
\tif (r) return std::optional<" << data.Name << ">{ std::in_place, *r };\\\n\
